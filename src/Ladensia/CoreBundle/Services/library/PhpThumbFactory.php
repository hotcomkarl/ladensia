 ...
 * @filesource
 */
namespace Ladensia\CoreBundle\Services\library;

use Ladensia\CoreBundle\Services\library\ThumbBase,
    Ladensia\CoreBundle\Services\library\PhpThumb,
    Ladensia\CoreBundle\Services\library\GdThumb;
...
class PhpThumbFactory
{
	...
	public static function create ($filename = null, $options = array(), $isDataStream = false)
	{
		...
		{
			$toReturn = new GdThumb($filename, $options, $isDataStream); //$implementationMap[self::$defaultImplemenation];
                        //var_dump($implementationMap[self::$defaultImplemenation]);die();
